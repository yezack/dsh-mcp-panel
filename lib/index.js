...
const CATALOG_SCHEMA = "dsh-mcp-panel/catalog@v1";
...
*/
function mergeCatalog(builtin, user) {
	const order = [];
...
*/
function catalogIssues(value, index) {
	const issues = [];
	const field = (name) => `catalog[${index}].${name}`;
	if (typeof value !== "object" || value === null || Array.isArray(value)) return [{
		field: `catalog[${index}]`,
		text: "catalog entry must be a JSON object"
	}];
	const entry = value;
	if (typeof entry["id"] !== "string" || !CATALOG_ID_PATTERN.test(entry["id"])) issues.push({
		field: field("id"),
		text: `id must match ${CATALOG_ID_PATTERN.source} and double as the serverName`
	});
	if (typeof entry["name"] !== "string" || entry["name"].trim() === "") issues.push({
		field: field("name"),
		text: "name must be a non-empty string"
	});
	if (typeof entry["description"] !== "string" || entry["description"].trim() === "") issues.push({
		field: field("description"),
		text: "description must be a non-empty string"
	});
	if (entry["transport"] !== "stdio" && entry["transport"] !== "streamable-http") issues.push({
		field: field("transport"),
		text: "transport must be \"stdio\" or \"streamable-http\""
	});
	if (entry["transport"] === "stdio" && (typeof entry["command"] !== "string" || entry["command"].trim() === "")) issues.push({
		field: field("command"),
		text: "stdio entries require a command"
	});
	if (entry["transport"] === "streamable-http") {
		const url = entry["url"];
		if (typeof url !== "string" || url === "") issues.push({
			field: field("url"),
			text: "streamable-http entries require a url"
		});
		else try {
			new URL(url);
		} catch {
			issues.push({
				field: field("url"),
				text: "url must be an absolute URL"
			});
		}
	}
	const checkKeys = (candidate, name) => {
		if (candidate === void 0) return;
		if (!Array.isArray(candidate) || candidate.some((key) => typeof key !== "string" || key.trim() === "")) issues.push({
			field: field(name),
			text: `${name} must be an array of non-empty strings`
		});
	};
...
function catalogOverlayIssues(value) {
	if (value === void 0) return [];
	if (!Array.isArray(value)) return [{
		field: "catalog",
		text: "catalogEntries must be an array of entries"
	}];
...
*/
function catalogToConfigInput(entry) {
	return {
		serverName: entry.id,
		transport: entry.transport,
		...entry.transport === "stdio" && entry.command !== void 0 ? { command: entry.command } : {},
		...entry.transport === "stdio" && entry.args !== void 0 && entry.args.length > 0 ? { args: [...entry.args] } : {},
		...entry.transport === "streamable-http" && entry.url !== void 0 ? { url: entry.url } : {}
	};
}
//#endregion
...
const Config = z.object({
	...
	trialMaxResultChars: z.number().min(1e3).max(MAX_TRIAL_RESULT_CHARS).default(DEFAULT_TRIAL_MAX_RESULT_CHARS),
	writeEnabled: z.boolean().default(true),
	backupCount: z.number().min(1).max(50).default(5),
...
function resolveConfig(config) {
	...
	const trialEnabled = config?.trialEnabled ?? true;
	if (typeof trialEnabled !== "boolean") throw new TypeError("dsh-mcp-panel: config.trialEnabled must be a boolean");
	const trialTimeoutMs = config?.trialTimeoutMs ?? 12e4;
	if (!Number.isFinite(trialTimeoutMs) || trialTimeoutMs < 1 || trialTimeoutMs > 6e5) throw new Error(`dsh-mcp-panel: config.trialTimeoutMs must be a finite number between 1 and ${MAX_TRIAL_TIMEOUT_MS}`);
	const trialMaxResultChars = config?.trialMaxResultChars ?? 6e4;
	if (!Number.isInteger(trialMaxResultChars) || trialMaxResultChars < 1e3 || trialMaxResultChars > 5e5) throw new Error(`dsh-mcp-panel: config.trialMaxResultChars must be an integer between 1000 and ${MAX_TRIAL_RESULT_CHARS}`);
	const writeEnabled = config?.writeEnabled ?? true;
	if (typeof writeEnabled !== "boolean") throw new TypeError("dsh-mcp-panel: config.writeEnabled must be a boolean");
	const backupCount = config?.backupCount ?? 5;
	if (!Number.isInteger(backupCount) || backupCount < 1 || backupCount > 50) throw new Error(`dsh-mcp-panel: config.backupCount must be an integer between 1 and 50`);
	const catalogOverlay = config?.catalogEntries ?? [];
	const catalogIssues = catalogOverlayIssues(catalogOverlay);
	if (catalogIssues.length > 0) throw new Error(`dsh-mcp-panel: config.catalogEntries invalid — ${catalogIssues.map((issue) => `${issue.field}: ${issue.text}`).join(" ")}`);
...
const EN_MESSAGES = {
	...
	noPatchForLeftover: (server) => `"${server}" is not a configured server — its tools come from another plugin's mcp__ namespace, so there is no row to disable or enable.`,
	...
	callResult: (tool, callId, ms, outcome) => `Trial call ${tool} through the official tool pipeline (${callId}, ${ms}ms): ${outcome}.`,
...
const MESSAGES = {
	...
	zh: {
		...
		noPatchForLeftover: (server) => `"${server}" 不是已配置的服务器——其工具来自其他插件的 mcp__ 命名空间，没有可停用/启用的条目。`,
		...
		callResult: (tool, callId, ms, outcome) => `试用调用 ${tool}（官方工具管线，${callId}，${ms}ms）：${outcome}。`,
	...
	es: {
		...
		noPatchForLeftover: (server) => `"${server}" no es un servidor configurado — sus herramientas provienen del espacio mcp__ de otro plugin, así que no hay fila que deshabilitar o habilitar.`,
		...
		callResult: (tool, callId, ms, outcome) => `Llamada de prueba ${tool} por el pipeline oficial de herramientas (${callId}, ${ms}ms): ${outcome}.`,
	...
	pt: {
		...
		noPatchForLeftover: (server) => `"${server}" não é um servidor configurado — suas ferramentas vêm do namespace mcp__ de outro plugin, então não há linha para desabilitar ou habilitar.`,
		usage: "Uso: /mcp | /mcp <server> | /mcp <server> tools | /mcp <server> call <tool> [json-argumentos] | /mcp <server> health | /mcp <server> disable | /mcp <server> enable | /mcp <server> probe",
		hint: "[server] [tools|call|health|disable|enable|probe]",
		...
		callUsage: "Uso: /mcp <server> call <tool> [json-argumentos]",
		callResult: (tool, callId, ms, outcome) => `Chamada de teste ${tool} pelo pipeline oficial de ferramentas (${callId}, ${ms}ms): ${outcome}.`,
	...
	hi: {
		...
		noPatchForLeftover: (server) => `"${server}" कॉन्फ़िगर किया गया सर्वर नहीं है — इसके टूल दूसरे प्लगइन के mcp__ नेमस्पेस से आते हैं, इसलिए अक्षम/सक्षम करने के लिए कोई पंक्ति नहीं है।`,
		...
		callResult: (tool, callId, ms, outcome) => `आधिकारिक टूल पाइपलाइन से परीक्षण कॉल ${tool} (${callId}, ${ms}ms): ${outcome}।`,
...
*/
function renderInventory(snapshot, messages = EN_MESSAGES) {
	const configLayers = snapshot.configLayers;
	if (configLayers === void 0 || !configLayers.scanned) return "";
	const extra = configLayers.entries.filter((entry) => !entry.effective);
	if (extra.length === 0) return "";
	const lines = [messages.inventoryHeader];
	for (const entry of extra) {
		const occurrences = entry.occurrences.map((occ) => {
...
function parseMcpArgs(rawInput) {
	...
	if (tokens.length !== 2) {
		if (action === "call") {
			...
			if (match === null || match[2] === void 0) return { kind: "usage" };
			return {
				kind: "server",
				server: match[1] ?? "",
				action: "call",
				tool: match[2],
				argsJson: (match[3] ?? "").trim()
			};
		}
		return { kind: "usage" };
	}
	if (action === "tools" || action === "disable" || action === "enable" || action === "probe" || action === "health") return {
		kind: "server",
		server: server ?? "",
		action
	};
...
function renderTrialCall(tool, result, messages = EN_MESSAGES) {
	...
	return `${header}\n${body.length > TRIAL_RENDER_CAP ? `${body.slice(0, TRIAL_RENDER_CAP)}… [truncated for display]` : body}`;
}
/** Render the health diagnostics block for one server. */
function renderHealth(view, messages = EN_MESSAGES) {
	const lines = [
		messages.healthHeader(view.serverName),
		renderServer(view, messages),
		""
	];
	...
	if (view.diagnostics.length === 0) lines.push(messages.healthNone);
	else {
	...
	return {
		...
		handler: ({ rawInput, agent }) => {
			...
			switch (parsed.action) {
				...
				case "health": return {
					kind: "success",
					text: renderHealth(view, messages)
				};
				case "call": {
					const request = {
					...
					return service.callTool(JSON.stringify(request), String(agent.id)).then((result) => {
						return {
							kind: "success",
							text: renderTrialCall(parsed.tool, result, messages)
						};
					}, (error) => ({
						kind: "error",
				...
				case "enable":
					if (view.entryId === "") return {
						kind: "error",
						text: messages.noPatchForLeftover(parsed.server)
					};
					return {
						kind: "success",
						text: renderPatchSuggestion(view, parsed.action, snapshot.patchFile, messages)
					};
				case "probe": try {
...
const PATTERNS = [
	...
	{
		pattern: /spawn|child process|process exited/iu,
		code: "command-spawn-failed",
		text: "The server process failed to start or exited — check the command line, its arguments, and the server's own logs."
	},
	{
		pattern: /ECONNREFUSED|connection refused/iu,
		code: "connection-refused",
		text: "The server refused the connection — start the server or check the host/port in the URL."
	},
	{
		pattern: /ECONNRESET|EPIPE|socket hang up|connection reset/iu,
		code: "connection-dropped",
		text: "The connection dropped mid-session — the server may have crashed or restarted."
	},
	{
		pattern: /ETIMEDOUT|timed? ?out|timeout|AbortError/iu,
		code: "timeout",
		text: "A request timed out — the server may be overloaded, or the URL may be wrong. Check toolCallTimeoutMs if calls are slow."
	},
	{
		pattern: /ENOTFOUND|EAI_AGAIN|getaddrinfo|name resolution/iu,
		code: "dns",
		text: "The server hostname did not resolve — check the DNS name in the URL and the network."
	},
	{
		pattern: /\b401\b|unauthorized/iu,
		code: "auth-401",
		text: "The server rejected the credentials (HTTP 401) — check the Authorization header or token."
	},
	{
		pattern: /\b403\b|forbidden/iu,
		code: "auth-403",
		text: "The server denied access (HTTP 403) — the credential is valid but lacks permission."
	},
	{
		pattern: /\b404\b|not found/iu,
		code: "path-404",
		text: "The endpoint path returned 404 — check the URL path (e.g. /mcp)."
	},
	{
		pattern: /\b429\b|rate limit/iu,
		code: "rate-limit",
		text: "The server rate-limited the client — wait, or reduce the reconnect/request rate."
	},
	{
		pattern: /EACCES|EPERM|permission denied|not permitted/iu,
		code: "permission",
		text: "A permission error occurred — check file permissions, PATH access, and whether the command is executable."
	}
];
/**
...
*/
function diagnoseServer(facts) {
	const suggestions = [];
	if (facts.lastError !== null && facts.lastError !== "") {
		for (const { pattern, code, text } of PATTERNS) if (pattern.test(facts.lastError)) {
			suggestions.push({
				code,
				text
			});
	...
	if (facts.phase === "exhausted" || facts.attempt >= 0 && facts.maxAttempts > 0 && facts.attempt >= facts.maxAttempts) suggestions.push({
	...
	else if (facts.phase === "waiting") suggestions.push({
	...
	if (facts.fiberPhase === "failed") suggestions.push({
		code: "entry-failed",
		text: "The cordis row failed to mount — read the loader diagnostics, then fix the row and reload."
	});
	if (facts.probeState === "unreachable") suggestions.push({
		code: "connection-refused",
...
const MCP_CLIENT_MODULE$1 = "@deepseek-ai/dsh-mcp-client";
...
*/
function configViewOf(config, fallbackName) {
	const transport = stringField(config, "transport", "");
	const normalized = transport === "stdio" || transport === "streamable-http" ? transport : "unknown";
	const argsValue = plainField(config, "args");
	const args = Array.isArray(argsValue) ? argsValue.filter((entry) => typeof entry === "string") : [];
	const keysOf = (key) => {
		const value = plainField(config, key);
		if (typeof value !== "object" || value === null || Array.isArray(value)) return [];
	...
	const reconnectValue = plainField(config, "reconnect");
	let reconnectEnabled = null;
	let reconnectMaxAttempts = null;
	if (typeof reconnectValue === "object" && reconnectValue !== null && !Array.isArray(reconnectValue)) {
		const enabled = plainField(reconnectValue, "enabled");
		const maxAttempts = plainField(reconnectValue, "maxAttempts");
...
function aggregateServerView(row, serverName, group, facts) {
	...
	const stderrTail = status?.stderrTail === void 0 ? null : sanitizeError(status.stderrTail);
	return {
		serverName,
		entryId: row?.entryId ?? "",
		transport,
		...
		config: row === void 0 ? null : configViewOf(row.config, `entry:${row.entryId}`),
...
function validateServerConfig(input) {
	if (typeof input !== "object" || input === null || Array.isArray(input)) return {
		...
		issues: [{
			...
			text: "Server config must be a JSON object."
		}]
	};
	const value = input;
	const issues = [];
	const serverName = typeof value["serverName"] === "string" ? value["serverName"] : "";
	if (serverName === "") issues.push({
		code: "serverName-required",
		text: "serverName is required."
	});
	else if (!SERVER_NAME_PATTERN.test(serverName)) issues.push({
		code: "serverName-format",
		text: "serverName must match [A-Za-z0-9_-]{1,32}."
	});
	const transport = value["transport"];
	if (transport !== "stdio" && transport !== "streamable-http") issues.push({
		code: transport === void 0 ? "transport-required" : "transport-unknown",
		text: "transport must be \"stdio\" or \"streamable-http\"."
	});
	if (transport === "stdio" && (typeof value["command"] !== "string" || value["command"].trim() === "")) issues.push({
		code: "command-required",
		text: "stdio servers require a command."
	});
	if (transport === "streamable-http") {
		const url = value["url"];
		if (typeof url !== "string" || url === "") issues.push({
			code: "url-required",
			text: "streamable-http servers require a URL."
		});
		else try {
			new URL(url);
		} catch {
	...
	if (timeout !== void 0 && (!Number.isInteger(timeout) || timeout < 1)) issues.push({
		code: "timeout-invalid",
		text: "toolCallTimeoutMs must be a positive integer."
	});
	const reconnectAttempts = value["reconnectMaxAttempts"];
	if (reconnectAttempts !== void 0 && (!Number.isInteger(reconnectAttempts) || reconnectAttempts < 1)) issues.push({
		code: "reconnect-invalid",
		text: "reconnectMaxAttempts must be a positive integer."
	});
	const checkMap = (candidate, code, label) => {
		if (candidate === void 0) return;
		if (typeof candidate !== "object" || candidate === null || Array.isArray(candidate)) {
			issues.push({
				code,
				text: `${label} must be a string map.`
			});
			return;
		}
		for (const [key, entryValue] of Object.entries(candidate)) if (key.trim() === "" || typeof entryValue !== "string") {
			issues.push({
				code,
				text: `${label} keys must be non-empty and values must be strings.`
			});
			return;
		}
	};
	...
	const checkedTransport = transport;
	return {
		ok: true,
		config: {
			serverName,
			...
			...Array.isArray(value["args"]) ? { args: value["args"].filter((entry) => typeof entry === "string") } : {},
			...typeof value["cwd"] === "string" && value["cwd"] !== "" ? { cwd: value["cwd"] } : {},
			...checkedTransport === "streamable-http" && typeof value["url"] === "string" ? { url: value["url"] } : {},
			...Number.isInteger(timeout) && timeout > 0 ? { toolCallTimeoutMs: timeout } : {},
			...value["failOnStartupError"] === true ? { failOnStartupError: true } : {},
			...value["reconnectEnabled"] === false ? { reconnectEnabled: false } : {},
			...Number.isInteger(reconnectAttempts) && reconnectAttempts > 0 ? { reconnectMaxAttempts: reconnectAttempts } : {},
			...typeof value["env"] === "object" && value["env"] !== null ? { env: value["env"] } : {},
			...Array.isArray(value["keepEnv"]) ? { keepEnv: value["keepEnv"].filter((entry) => typeof entry === "string") } : {},
			...typeof value["headers"] === "object" && value["headers"] !== null ? { headers: value["headers"] } : {},
			...Array.isArray(value["keepHeaders"]) ? { keepHeaders: value["keepHeaders"].filter((entry) => typeof entry === "string") } : {}
		}
	};
}
/** Read a raw string-map field from serialized loader config (never evaluates `!!js`). */
function rawStringMap(config, key) {
	if (typeof config !== "object" || config === null || Array.isArray(config)) return {};
	const value = config[key];
	if (typeof value !== "object" || value === null || Array.isArray(value)) return {};
	const result = {};
	for (const [name, entry] of Object.entries(value)) if (typeof entry === "string") result[name] = entry;
	return result;
}
/** Read a raw string field from serialized loader config. */
function rawString(config, key) {
	if (typeof config !== "object" || config === null || Array.isArray(config)) return void 0;
	const value = config[key];
...
*/
function mergeServerConfig(raw, input, view) {
	const unchanged = (editorValue, displayValue) => {
		if (editorValue === void 0) return true;
	...
	if (input.transport === "stdio") {
		...
		if (cwd !== void 0 && cwd !== "") merged["cwd"] = cwd;
		merged["env"] = mergeStringMap(rawStringMap(raw, "env"), input.env, input.keepEnv);
	} else {
		merged["url"] = (unchanged(input.url, view.url) ? rawString(raw, "url") ?? "" : input.url) ?? "";
		merged["headers"] = mergeStringMap(rawStringMap(raw, "headers"), input.headers, input.keepHeaders);
	}
	const timeout = unchanged(input.toolCallTimeoutMs, view.toolCallTimeoutMs) ? rawNumber(raw, "toolCallTimeoutMs") : input.toolCallTimeoutMs;
	if (timeout !== void 0) merged["toolCallTimeoutMs"] = timeout;
	if ((unchanged(input.failOnStartupError, view.failOnStartupError) ? rawBoolean(raw, "failOnStartupError") : input.failOnStartupError) === true) merged["failOnStartupError"] = true;
	const reconnectEnabled = unchanged(input.reconnectEnabled, view.reconnectEnabled) ? rawBoolean(raw, "reconnectEnabled") : input.reconnectEnabled;
	const reconnectAttempts = unchanged(input.reconnectMaxAttempts, view.reconnectMaxAttempts) ? rawNumber(raw, "reconnectMaxAttempts") : input.reconnectMaxAttempts;
	const reconnect = {};
	if (reconnectEnabled !== void 0) reconnect["enabled"] = reconnectEnabled;
	if (reconnectAttempts !== void 0) reconnect["maxAttempts"] = reconnectAttempts;
	if (Object.keys(reconnect).length > 0) merged["reconnect"] = reconnect;
	return merged;
}
/** Merge one string map with keep semantics. */
function mergeStringMap(raw, entries, keep) {
	const kept = new Set(keep ?? []);
	const result = {};
	for (const [key, rawValue] of Object.entries(raw)) if (kept.has(key)) result[key] = rawValue;
...
function rawStringArray(config, key) {
	if (typeof config !== "object" || config === null || Array.isArray(config)) return [];
	const value = config[key];
	if (!Array.isArray(value)) return [];
...
function materializeConfig(input) {
	...
	if (input.transport === "stdio") {
		...
		if (input.cwd !== void 0 && input.cwd !== "") config["cwd"] = input.cwd;
	...
	if (input.toolCallTimeoutMs !== void 0) config["toolCallTimeoutMs"] = input.toolCallTimeoutMs;
	if (input.failOnStartupError === true) config["failOnStartupError"] = true;
	const reconnect = {};
	if (input.reconnectEnabled !== void 0) reconnect["enabled"] = input.reconnectEnabled;
	if (input.reconnectMaxAttempts !== void 0) reconnect["maxAttempts"] = input.reconnectMaxAttempts;
	if (Object.keys(reconnect).length > 0) config["reconnect"] = reconnect;
	return config;
}
/** Default entry id the console assigns to a new server row. */
function defaultEntryId(serverName) {
	return `mcp-${serverName}`;
}
/** A fresh unique entry id for `add` given the ids already in the profile. */
function nextEntryId(serverName, existingIds) {
...
*/
function resolvePatchOp(op, rawFor, viewFor, existingIds) {
	if (typeof op !== "object" || op === null || Array.isArray(op)) return {
		ok: false,
		issues: [{
			code: "config-malformed",
			text: "Patch operation must be a JSON object."
		}]
	};
	const candidate = op;
	...
	if (kind === "edit" || kind === "disable" || kind === "enable") {
		...
		if (typeof entryId !== "string" || entryId === "") return {
			...
			issues: [{
				code: "config-malformed",
				text: `patch "${kind}" requires the row's entryId.`
			}]
		};
		if (!rawFor.has(entryId)) return {
			ok: false,
			issues: [{
				code: "config-malformed",
				text: `unknown entryId "${entryId}".`
			}]
		};
		if (kind === "disable" || kind === "enable") return {
			ok: true,
			op: {
				kind,
				entryId
		...
		if (view === void 0) return {
			...
			issues: [{
				code: "config-malformed",
				text: `no display view for "${entryId}".`
			}]
		};
		return {
			ok: true,
			op: {
				kind: "edit",
				entryId,
				rowConfig: mergeServerConfig(rawFor.get(entryId), validated.config, view)
			}
		};
	}
	return {
		...
		issues: [{
			code: "config-malformed",
			text: `unknown patch kind ${JSON.stringify(kind)}.`
		}]
	};
}
/** A one-line comment stamp marking a console-generated operation block. */
function patchComment(op, now = /* @__PURE__ */ new Date()) {
...
*/
function renderPatchFragment(op, now = /* @__PURE__ */ new Date()) {
	const lines = [`# ${patchComment(op, now)}`];
	switch (op.kind) {
		case "add":
...
function emitBlock(lines, keyLine, value, indent) {
...
function emitValue(lines, key, value, indent) {
	...
	if (Array.isArray(value)) {
		...
		lines.push(`${key}:`);
		for (const entry of value) if (typeof entry === "string") lines.push(`${" ".repeat(indent + 2)}- ${yamlScalar(entry)}`);
		else lines.push(`${" ".repeat(indent + 2)}- ${JSON.stringify(entry)}`);
		return;
	}
	if (typeof value === "object" && value !== null) {
		emitBlock(lines, `${key}:`, value, indent + 2);
		return;
	}
...
*/
async function appendPatchFragment(filePath, fragment, backupCount) {
	let existing = true;
	try {
...
async function appendFragment(filePath, fragment) {
...
async function pruneBackups(filePath, keep) {
	...
	const sorted = names.map((name) => ({
		name,
		stamp: Number(name.slice(prefix.length))
	})).filter((entry) => Number.isFinite(entry.stamp)).sort((left, right) => right.stamp - left.stamp);
	for (const entry of sorted.slice(keep)) try {
...
function validateTrialRequest(request) {
	if (typeof request !== "object" || request === null || Array.isArray(request)) return "trial request must be a JSON object";
...
function createTrialCaller() {
	...
	*/
async runTrialCall(tools, agents, sessionId, request, limits) {
		const { serverName, toolName } = request;
		const prefix = `mcp__${serverName}__`;
		if (!toolName.startsWith(prefix)) throw new Error(`tool "${toolName}" does not belong to server "${serverName}" (expected the ${prefix}… namespace)`);
		if (tools.get(toolName) === void 0) throw new Error(`tool "${toolName}" is not registered — the server may be down or its sync failed`);
		let argumentsValue;
		try {
		...
		const agent = sessionId === void 0 || sessionId === "" ? void 0 : agents?.get(sessionId);
		const callId = `${TRIAL_CALL_PREFIX}-${++trialCounter}`;
		const started = Date.now();
		const execution = {
			callId,
			name: toolName,
...
function projectTrialResult(result, budget) {
	const { capped, truncated } = capJsonValue(result.isError ? {
		error: {
			message: result.error.message,
			...result.error.info === void 0 ? {} : { code: result.error.info.code }
		},
...
function capJsonValue(value, budget) {
	if (budget <= 0) return {
		capped: CAP_MARKER,
		truncated: true
	};
	if (typeof value === "string") {
		if (value.length <= budget) return {
			capped: value,
			truncated: false
		};
		return {
			capped: `${value.slice(0, budget)}…`,
			truncated: true
		};
	}
...
const MCP_CONFIG_EXPORT_SCHEMA = "dsh-mcp-panel/mcp-config@v1";
...
*/
function exportMcpConfigs(servers) {
	const rows = servers.map(({ entryId, config }) => {
		if (!isPlainJson(config) || typeof config !== "object" || config === null || Array.isArray(config)) return {
			entryId,
			config: null,
			reason: "config carries a !!js expression or is not a plain object; re-create it in the editor"
		};
		return {
			entryId,
			config
	...
	return `${JSON.stringify({
		schema: MCP_CONFIG_EXPORT_SCHEMA,
		servers: rows
	}, null, 2)}\n`;
}
/**
...
*/
function parseMcpConfigsImport(text) {
	let document;
	try {
	...
	if (typeof document !== "object" || document === null || Array.isArray(document)) throw new Error("dsh-mcp-panel: import must be a JSON object");
	const servers = document["servers"];
	if (!Array.isArray(servers) || servers.length === 0) throw new Error("dsh-mcp-panel: import must carry a non-empty \"servers\" array");
	const out = [];
	for (let index = 0; index < servers.length; index += 1) {
		const entry = servers[index];
		if (typeof entry !== "object" || entry === null || Array.isArray(entry)) throw new Error(`dsh-mcp-panel: import servers[${index}] must be an object`);
		const record = entry;
		const config = record["config"];
		if (config === null) throw new Error(`dsh-mcp-panel: import servers[${index}] has no config (exported as null)`);
		const validated = validateServerConfig(config);
		if (!validated.ok) throw new Error(`dsh-mcp-panel: import servers[${index}] invalid — ${validated.issues.map((issue) => issue.text).join(" ")}`);
		const entryId = typeof record["entryId"] === "string" && record["entryId"] !== "" ? record["entryId"] : defaultEntryId(validated.config.serverName);
		out.push({
			entryId,
...
const PROFILE_PATCH_FILENAME$1 = "cordis.patch.yml";
...
function profileLayerSpecs(baseDir) {
	return [{
		layer: "profile-cordis",
		label: `${PROFILE_LABEL_PREFIX}${PROFILE_BASE_FILENAME}`,
		file: join(baseDir, PROFILE_BASE_FILENAME)
	}, {
		layer: "profile-patch",
		label: `${PROFILE_LABEL_PREFIX}${PROFILE_PATCH_FILENAME$1}`,
		file: join(baseDir, PROFILE_PATCH_FILENAME$1)
	}];
}
/**
...
*/
function agentPresetSpecs(baseDir) {
	if (baseDir === null) return [];
	const presetsRoot = [join(baseDir, "..", "..", ".agent-presets"), join(baseDir, ".agent-presets")].find((dir) => existsSync(dir));
	if (presetsRoot === void 0) return [];
	const specs = [];
	for (const entry of readdirSync(presetsRoot, { withFileTypes: true })) {
		...
		specs.push({
			layer: "agent-preset",
			label: presetDisplayName(presetDir) ?? entry.name,
			file
		});
	}
...
function parseYamlText(text) {
	return (0, import_dist.parse)(text, {
		schema: "core",
		customTags: [{
...
function isMcpClientEntry(raw) {
	if (!isObject(raw)) return false;
	return raw["name"] === MCP_CLIENT_MODULE$1;
}
/**
...
*/
function extractRowsFromLayer(doc) {
	...
	for (const entry of doc) {
		...
		if (Array.isArray(insert)) {
			for (const item of insert) if (isMcpClientEntry(item)) ingest(item);
			continue;
		}
		...
		if (typeof entry["id"] === "string" && entry["id"] !== "" && "disabled" in entry) bareOverrides.push({
			entryId: entry["id"],
...
function effectiveNamesOf(rows) {
...
*/
function scanConfigLayers(baseDir, loaderRows = []) {
	if (baseDir === null) return {
		scanned: false,
		error: null,
		entries: []
	};
	const specs = [...profileLayerSpecs(baseDir), ...agentPresetSpecs(baseDir)];
	...
	const entries = [...perName.entries()].map(([serverName, occurrences]) => {
		return {
			serverName,
			profileVisible: occurrences.some((occ) => occ.layer === "profile-cordis" || occ.layer === "profile-patch"),
...
*/
const KNOWN_PHASES = /* @__PURE__ */ new Set([
	"connecting",
	"connected",
	"waiting",
	"exhausted",
...
const KNOWN_JOB_STATUSES = /* @__PURE__ */ new Set([
	"running",
	"stopping",
	"completed",
	"killed",
...
var McpPanelService = class extends TypertRemoteService {
	...
	* counts a reconnect.
	*/
	countedAttempts = /* @__PURE__ */ new Map();
	...
	*/
	observe(payload) {
		...
		const clean = {
			...
			...Number.isFinite(payload.delayMs) ? { delayMs: Math.floor(payload.delayMs) } : {},
			...typeof payload.error === "string" ? { error: payload.error } : {},
			...Number.isFinite(payload.connectedAt) ? { connectedAt: Math.floor(payload.connectedAt) } : {},
			...Number.isFinite(payload.exitCode) ? { exitCode: Math.floor(payload.exitCode) } : {},
		...
		} else if (clean.phase === "connected" || clean.phase === "disposed") this.countedAttempts.set(serverName, 0);
	}
	/**
	...
	*/
	status() {
		...
		const groups = groupMcpTools(this.ctx.tools.schemas(), rows.map((row) => serverNameOf(row.config, `entry:${row.entryId}`)));
	...
	*/
	async writePatch(opJson, confirmed, sessionId) {
		if (!this.config.writeEnabled) throw new Error("dsh-mcp-panel: profile writes are disabled (config.writeEnabled: false) — flip the kill switch or edit the file by hand");
		const file = this.patchFile();
		if (file === null) throw new Error("dsh-mcp-panel: the profile patch path is unknown (no ctx.baseUrl) — cannot write");
		const resolution = this.resolveOp(opJson);
		if (!resolution.ok) throw this.issueError(resolution);
		...
		if (approval !== void 0 && agent !== void 0 && hasOpenTurn(agent)) {
			...
			if (outcome === ALLOWED_ONCE) approvalPath = "harness-approval";
			else if (outcome === "rejected") throw new Error("dsh-mcp-panel: the profile write was rejected");
			else if (outcome === "cancelled") throw new Error("dsh-mcp-panel: approval for the profile write was cancelled");
			else throw new Error("dsh-mcp-panel: no approval channel is available for the profile write");
		}
		if (approvalPath === null) {
			if (confirmed !== true) throw new Error("dsh-mcp-panel: the profile write needs confirmation — review the fragment and confirm in the console");
	...
	*/
	async callTool(requestJson, sessionId) {
		if (!this.config.trialEnabled) throw new Error("dsh-mcp-panel: the tool trial console is disabled (config.trialEnabled: false)");
		let request;
		try {
		...
		return this.trialCaller.runTrialCall(this.ctx.tools, agentsOf(this.ctx), sessionId, request, {
			timeoutMs: this.config.trialTimeoutMs,
			maxResultChars: this.config.trialMaxResultChars
		});
	}
	...
	*/
	importPreview(json) {
		return parseMcpConfigsImport(json).map(({ entryId, config }) => {
			const resolution = this.resolveOp(JSON.stringify({
				kind: "add",
				config
			}));
			if (!resolution.ok) throw this.issueError(resolution);
	...
	resolveOp(opJson) {
		...
		const rawFor = /* @__PURE__ */ new Map();
		const viewFor = /* @__PURE__ */ new Map();
	...
	*/
	...
	probeViews() {
		...
		return jobs.list().filter((job) => job.kind === PROBE_KIND).map((job) => ({
			...
			status: KNOWN_JOB_STATUSES.has(job.status) ? job.status : "unknown",
...
function agentsOf(ctx) {
	...
	if (typeof agents !== "object" || agents === null) return void 0;
	return agents;
}
//#endregion
...
//#endregion
export { CATALOG_SCHEMA, Config, DEFAULT_CATALOG, MCP_CONFIG_EXPORT_SCHEMA, MCP_STATUS_EVENT, McpPanelService, aggregateServerView, aggregateSnapshot, appendPatchFragment, apply, catalogIssues, catalogOverlayIssues, catalogToConfigInput, configViewOf, countServerTools, createTrialCaller, defaultEntryId, deriveTarget, diagnoseServer, exportMcpConfigs, groupMcpTools, inject, mcpCommand, mcpProbeTool, mergeCatalog, mergeServerConfig, name, nextEntryId, parseMcpArgs, parseMcpConfigsImport, probeEndpoint, probeJob, renderHealth, renderList, renderPatchFragment, renderPatchSuggestion, renderServer, renderTools, renderTrialCall, resolveConfig, resolvePatchOp, sanitizeError, sanitizeText, sanitizeUrl, serverNameOf, validateServerConfig, validateTrialRequest, yamlScalar };
